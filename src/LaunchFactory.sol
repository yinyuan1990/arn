...
///
///      Arm: every launch gets its own CreatorFeeSplitter (via ReferralHub) registered as the lock's payout, so the
///      creator can share fees with promoters (`referralBps`, default 0 = off) without any extra transaction.
contract LaunchFactory is Ownable, ReentrancyGuard, IUniswapV3SwapCallback {
    ...
    uint16 public constant CREATOR_SHARE_BPS = 7_800;
    ...
    // ---------------------------------------------------------------- fixed economics
    /// @dev Launching is free. Kept as a getter (always 0) for the indexer / web ABI.
    uint256 public constant creationFee = 0;
    ...
    // Nothing about a launch is tunable. There is no setter and no owner-only function left on
    ...
    struct LaunchParams {
        ...
        uint16 referralBps; // share of the creator fees offered to promoters (0 = off, ≤ 5000); changeable later
    }
    ...
    constructor(
        ...
        if (
            uniFactory_ == address(0) || positionManager_ == address(0) || router_ == address(0) || usdc_ == address(0)
                || locker_ == address(0) || treasury_ == address(0) || referralHub_ == address(0)
        ) revert ZeroAddress();
        ...
        treasury = treasury_;
        referralHub = ReferralHub(referralHub_);
    }
    ...
    function launch(LaunchParams calldata p) external nonReentrant returns (address token, address pool, uint256 positionId) {
        ...
        // creator share → this token's splitter, which forwards to the creator wallet minus any referral pool
        address splitter = referralHub.create(token, p.payout == address(0) ? msg.sender : p.payout, p.referralBps);
        locker.register(token, usdc, pool, positionId, msg.sender, splitter, CREATOR_SHARE_BPS);
