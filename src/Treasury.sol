...
/// @title Treasury
/// @notice Collects the protocol share of swap fees (0.22% of every trade, i.e. 22% of the 1% pool fee) in USDC
///         and settles it on a fixed weekly cycle to three fixed addresses, in exact parts of 22:
///           - ECO_PARTS (16/22 = 0.16% of the trade) → the reserve;
///           - BUYBACK_PARTS (5/22 = 0.05% of the trade) → the buyback fund (whatever it buys back and burns is
///             done there by hand and recorded off-chain);
///           - DEV_PARTS (1/22 = 0.01% of the trade) → the development team wallet.
///         Together with the creator's 78% that is the whole 1% pool fee: 78 / 16 / 5 / 1.
...
contract Treasury is Ownable, ReentrancyGuard {
    ...
    uint256 public constant ECO_PARTS = 16; // → reserve
    uint256 public constant BUYBACK_PARTS = 5; // → buyback fund
    uint256 public constant DEV_PARTS = 1; // → development team
    uint256 public constant TOTAL_PARTS = 22;
    /// @dev Rounded display values of the three parts (the math uses the exact parts above).
    uint16 public constant ECO_BPS = 7_272;
    uint16 public constant BUYBACK_BPS = 2_273;
    uint16 public constant DEV_BPS = 455;
    ...
    /// @notice Weekly settlement. Permissionless. 16/22 of new revenue → reserve, 5/22 → buyback fund,
    ///         1/22 → dev team. Rounding dust stays for the next cycle.
    function execute() external nonReentrant {
        ...
        uint256 toEco = (fresh * ECO_PARTS) / TOTAL_PARTS;
        uint256 toBuyback = (fresh * BUYBACK_PARTS) / TOTAL_PARTS;
        uint256 toDev = (fresh * DEV_PARTS) / TOTAL_PARTS;
