...
abstract contract Base is Test {
    ...
    function setUp() public virtual {
        ...
        hub = new ReferralHub(address(usdc), address(locker), keeper, owner);
        factory = new LaunchFactory(
            address(uni), address(nfpm), address(router), address(usdc), address(locker), address(treasury), address(hub), owner
        );
        vm.startPrank(owner);
    ...
    function splitterOf(address token) internal view returns (CreatorFeeSplitter) {
        return CreatorFeeSplitter(hub.splitterOf(token));
    }

    /// @dev FeeLocker pushes the creator share to the token's splitter; `sync` forwards it to the creator
    ///      (all of it when referralBps = 0). This is what the keeper does back to back.
    function distributeAndSync(address token, uint256 minOut) internal returns (uint256 usdcCollected, uint256 usdcFromToken) {
        (usdcCollected, usdcFromToken) = locker.distribute(token, minOut);
        splitterOf(token).sync();
    }
